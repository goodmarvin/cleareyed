...
export default function Chat() {
  ...
  const { messages, input, handleInputChange, handleSubmit, isLoading } =
    useChat({
      api: '/api/query', // Point to our Clear-Eyed API
      maxSteps: 2, // Reduced since we don't need multi-step for now
      onToolCall({ toolCall }) {
  ...
  const userScenario: Message | undefined = messages
    .filter((m) => m.role === "user")
    .slice(-1)[0];

  const lastInsightMessage: Message | undefined = messages
    .filter((m) => m.role !== "user")
    .slice(-1)[0];
                ...
                {awaitingResponse || currentToolCall ? (
                  <div className="px-2 min-h-12">
                    <div className="dark:text-neutral-400 text-neutral-500 text-sm w-fit mb-1">
                      Your scenario: {userScenario?.content}
                    </div>
                  ...
                  <div className="px-2 min-h-12">
                    ...
                    <InsightMessage message={lastInsightMessage} />
                  </div>
                ) : null}
              </AnimatePresence>
            </motion.div>
...
const InsightMessage = ({ message }: { message: Message | undefined }) => {
...
const Loading = ({ tool }: { tool?: string }) => {
  const loadingMessage = tool 
    ? `Analyzing with ${tool}...`
    : "Analyzing your scenario and finding relevant mental models...";
